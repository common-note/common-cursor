...
export class Formatter {
  ...
  _inTag(anchor: Anchor, tag: string): HTMLElement | null {
    ...
    while (current) {
      if (
        ...
        current.tagName.toLowerCase() === tag
      ) {
        return current;
      }
  ...
  _concatText(node: Text, direction: 'left' | 'right'): boolean {
    if (direction === 'left') {
      let prev = this.editor._getNeighborSibling({
        container: node,
      ...
      if (prev && prev instanceof Text) {
        ...
        prev.remove();
        prev = this.editor._getNeighborSibling({
          container: node,
          step: { direction: 'left', stride: 'char' },
        });
      }
    ...
    let next = this.editor._getNeighborSibling({
      container: node,
    ...
    if (next && next instanceof Text) {
      ...
      next.remove();
      next = this.editor._getNeighborSibling({
        container: node,
        step: { direction: 'right', stride: 'char' },
      });
    }
  ...
  _unsurrondNode(node: Element): FormatResult {
    ...
    if (shouldIgnore) {
      return {
        start: this.editor.getBoundaryAnchorOutsideNode({
          container: nodes[0],
          step: { direction: 'left', stride: 'char' },
        }),
        end: this.editor.getBoundaryAnchorOutsideNode({
  ...
  format(tag: string): FormatResult {
    ...
    if (range.collapsed) {
      ...
      return this._expandWord(tag, {
        container: range.start.container,
